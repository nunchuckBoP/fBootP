...
class DhcpPacket(DhcpBasicPacket):
        ...
        for opt in  ['htype','hlen','hops','xid','secs','flags',
            ...
            if DhcpFieldsTypes[opt] == "int" : result = str(data[0])
                ...
                for iterator in range(6) :
                    p1 = math.floor(data[iterator] / 16)
                    p0 = math.floor(data[iterator] % 16)
                    result += [str(hexsym[p1]+hexsym[p0])]
    ...
    def GetMacAddressString(self):
        h = self.GetHardwareAddress()
        return ":".join([str(hex(i)[2:]).upper() for i in h])
